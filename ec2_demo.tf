resource "aws_security_group" "beats_sg" {
  ...
  ingress {                                     # is this needed?
    description      = "for access of Logstash"
    from_port        = 5044                     
    to_port          = 5044
    protocol         = "tcp"
...
resource "aws_instance" "filebeat_ec2" {
    ...
    user_data=templatefile("${path.module}/user-data-filebeat.sh",
          {
            LOGSTASH_IP = aws_instance.logstash_ec2.ip
          })
    tags = {
